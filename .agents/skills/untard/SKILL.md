...
> Untard asks _should this thing exist at all, and is it paying rent?_ If it's net-negative drag,
...
Walk it: _"what forces this?"_ → answer → _"is THAT in our control?"_ → repeat. In a one-dev +
agents shop, almost every "hard constraint" turns out to be a setting, a default, a convention, or a
hack papering over a collision of two of your _own_ choices. Keep walking until you hit something
genuinely outside your control — a protocol, a vendor API, physics. Everything above that line is
negotiable. So renegotiate it.

If the only thing forcing the pain is something _you_ set: stop obeying it. We do it our way.
...
2. **Tally the cost.** What does it cost us _continuously_? Drag, ceremony, a second moving part, a
...
4. **Net it out.** Benefit minus cost-and-risk. If it isn't _clearly_ positive, it's dead weight.
...
- Something everyone routes _around_ instead of _through_ — that's the tell it shouldn't be there.
...
- Swinging to the opposite extreme. Untard removes _net-negative_ drag; it does **not** ban all
...
- Deleting something you never actually netted out. Do the tally, _then_ cut.

## Relationship to /upstream

- `/upstream`: a problem exists → trace to source → fix it THERE.
- `/untard`: a _thing_ exists → does it deserve to → if not, DELETE it, then upstream the gap.
...
canon, backed by a gate that wanted that file _present_. It looked load-bearing. It was pure
...
The "constraint" forcing it — _"a repo must reach the shared canon from inside itself"_ — was a
...
the canon by _identity_, never require the file present, never carry a copy — **while `_ops` stays
the unbounded single source**, the brain that holds the full text. Risk drops exponentially (no
cross-repo file coupling left to break) and nothing duplicates (no drift). The repo _names_ the rule;
`_ops` _owns_ the rule.
